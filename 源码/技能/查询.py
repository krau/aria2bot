...
查询活跃刷新标记 = InlineKeyboardMarkup(
    inline_keyboard=[
        [
            InlineKeyboardButton("刷新", callback_data="刷新活跃任务"),
...
async def 刷新活跃任务(更新: Update, 上下文: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await 上下文.bot.edit_message_text(
            chat_id=更新.effective_chat.id,
            message_id=更新.callback_query.message.message_id,
            text=f"当前活跃任务:\n\n{好结果} 共有{len(结果)}个活跃任务",
            parse_mode="Markdown",
            reply_markup=查询活跃刷新标记,
        )
        ...
        await 上下文.bot.send_message(
            chat_id=更新.effective_chat.id,
            text=f"刷新活跃任务出错,错误信息:\n {e.__class__.__name__}: {e}",
            reply_to_message_id=更新.effective_message.id,
        )


查询等待刷新标记 = InlineKeyboardMarkup(
    inline_keyboard=[
        [
            InlineKeyboardButton("刷新", callback_data="刷新等待中任务"),
            InlineKeyboardButton("回主菜单", callback_data="回主菜单"),
        ],
    ]
)
...
async def 查询等待中任务(更新: Update, 上下文: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await 上下文.bot.send_message(
            chat_id=更新.effective_chat.id,
            text=f"当前等待中任务共{len(结果)}个,最多显示前10条:\n\n{好结果}",
            parse_mode="Markdown",
            reply_markup=查询等待刷新标记,
            reply_to_message_id=更新.effective_message.id,
        )
    except Exception as e:
...
async def 回主菜单(更新: Update, 上下文: ContextTypes.DEFAULT_TYPE):
    await 上下文.bot.delete_message(
        chat_id=更新.effective_chat.id, message_id=更新.callback_query.message.message_id
    )
    await 上下文.bot.send_message(
        chat_id=更新.effective_chat.id, text="请选择操作", reply_markup=开始标记
    )


查询下载器刷新标记 = InlineKeyboardMarkup(
    [
        [
            InlineKeyboardButton("刷新", callback_data="刷新下载器状态"),
            InlineKeyboardButton("回主菜单", callback_data="回主菜单"),
        ]
    ]
)
...
async def 查询下载器状态(更新: Update, 上下文: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await 上下文.bot.send_message(
            ...
            text=f"{好结果}",
            parse_mode="Markdown",
            reply_markup=查询下载器刷新标记,
            reply_to_message_id=更新.effective_message.id,
        )
    except Exception as e:
        await 上下文.bot.send_message(
            chat_id=更新.effective_chat.id,
            text=f"查询下载器状态出错,错误信息:\n {e.__class__.__name__}: {e}",
            reply_to_message_id=更新.effective_message.id,
        )
