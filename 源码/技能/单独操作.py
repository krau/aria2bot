...
from telegram.ext import ContextTypes, MessageHandler, filters, CallbackQueryHandler, ConversationHandler, CallbackDataCache, CallbackContext
from telegram.error import BadRequest
from aioaria2 import Aria2rpcException
from ..下载器 import 获取下载器
from ..日志 import 日志器
from ..小工具 import 仅主人装饰器, 有机体可读统计结果, 不是主人, 有机体可读下载任务详细结果
...
async def 暂停单任务(更新: Update, 上下文: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await 上下文.bot.edit_message_text(chat_id=更新.effective_chat.id, message_id=更新.callback_query.message.id,
                                        text=f'{新好任务状态}\n 已暂停任务', parse_mode='Markdown', reply_markup=操作单任务回复标记)
...
async def 继续单任务(更新: Update, 上下文: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        新好任务状态 = await 有机体可读下载任务详细结果(新任务状态)
        await 上下文.bot.edit_message_text(chat_id=更新.effective_chat.id, message_id=更新.callback_query.message.id,
                                        text=f'{新好任务状态}\n 已继续任务', parse_mode='Markdown', reply_markup=操作单任务回复标记)
    except Aria2rpcException:
        pass
    except Exception as e:
...
async def 刷新单任务(更新: Update, 上下文: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        好任务状态 = await 有机体可读下载任务详细结果(任务状态)
        await 上下文.bot.edit_message_text(chat_id=更新.effective_chat.id, message_id=更新.callback_query.message.id,
                                        text=f'{好任务状态}', parse_mode='Markdown', reply_markup=操作单任务回复标记)
    except BadRequest:
        await 上下文.bot.edit_message_text(chat_id=更新.effective_chat.id, message_id=更新.callback_query.message.id,
